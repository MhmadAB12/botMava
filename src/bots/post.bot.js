...
async function runAccountAndEngagementBot(postsPerRun = 3, delayBetween = 2000) {
  if (isRunning) {
    logger.warn('⚠️ Bot already running, skipping this run');
    return;
  }
  ...
  try {
    ...
    logger.info('💾 Account saved with cookies');

    /* ⏳ تهدئة النظام قبل الإغلاق */
    await wait(3000);
  } catch (err) {
  ...
  await mongoose.disconnect();
  logger.info('🎉 Engagement bot finished');
